...
the entire transcription. For a long session (ours is ~3 hr / 229 MB) with a
...
- **Tier 1** hosts the audio in **S3** and hands Deepgram a URL, so you never
  upload the 229 MB from your laptop. Still a normal synchronous request — enough
  for `diarize_model=v1` (see the finding in [`README.md`](README.md), section 2).
...
aws iam create-role --role-name coe-deepgram-callback-role \
  --assume-role-policy-document file://trust.json

# CloudWatch Logs permissions
aws iam attach-role-policy --role-name coe-deepgram-callback-role \
...
aws iam put-role-policy --role-name coe-deepgram-callback-role \
...
aws lambda create-function --function-name coe-deepgram-callback \
  --runtime python3.12 --handler lambda_function.handler \
  --role arn:aws:iam::<ACCOUNT_ID>:role/coe-deepgram-callback-role \
...
aws lambda create-function-url-config \
  --function-name coe-deepgram-callback --auth-type NONE

# Allow anonymous invokes of the Function URL
aws lambda add-permission --function-name coe-deepgram-callback \
...
```bash
aws lambda update-function-configuration --function-name coe-deepgram-callback \
